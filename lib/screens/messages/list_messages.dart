...
class ListMessageState extends State<ListMessages> {
  ...
  // CORREÇÃO: Mover a lógica de filtro para fora do build
  void _applyFilters() {
  ...
  Widget listCardsMessages(List<Message> messages, BuildContext context, double screenHeight) {
    final filteredMessages = messages.where((e) => e.id != messages.first.id).toList();

    if (filteredMessages.isEmpty) {
      return const SizedBox.shrink();
    }

    return Column(children: filteredMessages.map((item) => _buildMessageCard(item, screenHeight, context)).toList());
  }
  ...
  Widget buildListMessages(BuildContext context, double screenHeight) {
    return Observer(
      builder: (context) {
        ...
        // CORREÇÃO: Aplicar filtros fora do build
        _applyFilters();
        ...
        if (_messagesController.messages == null || _messagesController.messages!.isEmpty) {
          return Container(
            ...
            child: const Column(
              children: <Widget>[
                AutoSizeText('Nenhuma mensagem está disponível para este aluno', maxFontSize: 18, minFontSize: 16, textAlign: TextAlign.center),
        ...
        return Column(
          ...
          children: <Widget>[
            ...
            _buildMessageCard(firstMessage, screenHeight, context),
  ...
  Widget _buildFilterChip(String label, bool isSelected, Color activeColor, double screenHeight, String filterType) {
    return GestureDetector(
      ...
      child: Chip(
        backgroundColor: isSelected ? activeColor : const Color(0xffDADADA),
        avatar: isSelected ? Icon(FontAwesomeIcons.check, size: screenHeight * 2) : null,
        label: AutoSizeText(label, style: const TextStyle(color: Color(0xff42474A))),
      ),
    );
  }
    ...
    if (_messagesController.filteredList != null && _messagesController.filteredList!.isNotEmpty) {
      ...
      return Container(
        ...
        child: const AutoSizeText(
          ...
          minFontSize: 14,
          maxFontSize: 16,
          style: TextStyle(color: Color(0xff727374), fontWeight: FontWeight.w600),
        ),
      );
    } else {
      return Container(
